...
@SpringJUnitConfig
@DirtiesContext
class Mqtt3MessageHandlerTests implements HiveMQContainer {

	static final String CAR_DEVICE_DEFAULT_TOPIC = "mqtt-v3-outbound-car-default-topic";
	...
	void testCarDeviceMessageHandler_DefaultTopic_BytePayload() throws InterruptedException {
		...
		// When
		carDeviceInputChannel.send(MessageBuilder
				.withPayload("simple-car-payload".getBytes())
	...
	void testCarDeviceMessageHandler_HeaderTopic_StringPayload() throws InterruptedException {
		// Given
		String carTopic = "mqtt-v3-outbound-car-header-topic";
		CountDownLatch callbackLatch = new CountDownLatch(1);
		Consumer<Mqtt3Publish> consumer = mqtt3Publish -> {
			// Then
			Assertions.assertThat(mqtt3Publish)
	...
	void testCarDeviceMessageHandler_HeaderTopic_ObjectPayload() throws InterruptedException {
		// Given
		String carTopic = "mqtt-v3-outbound-car-header-topic-object-payload";
	...
	@Test
	void testCarDeviceMessageHandler_HeaderTopic_Mqtt3PublishPayload() throws InterruptedException {
		// Given
		String carTopic = "mqtt-v3-outbound-car-header-topic-mqtt3publish-payload";
		Mqtt3Publish mqtt3Payload = Mqtt3Publish.builder()
				.topic(carTopic)
		...
		mqtt3AsyncClient.subscribeWith()
				...
				.send();
		// When
		carDeviceInputChannel.send(MessageBuilder.withPayload(mqtt3Payload)
				.setHeader(MqttHeaders.TOPIC, carTopic)
				.build());
		// Await
		Assertions.assertThat(callbackLatch.await(10000, TimeUnit.MILLISECONDS)).isTrue();
	...
	void testPetDeviceMessageHandler_AsyncEvent() throws InterruptedException {
		// Given
		String petTopic = "mqtt-v3-outbound-pet-header-topic";
		// When
		...
		Assertions.assertThat(petDeviceMessageDeliveredEvent).isNotNull()
				...
				.isNotNull()
				.isInstanceOf(Mqtt3Publish.class);
	}
	...
	static class Config {
		...
		Mqtt3MessageHandler carDeviceMessageHandler(Mqtt3AsyncClient mqtt3AsyncClient) {
			var messageHandler = new Mqtt3MessageHandler(mqtt3AsyncClient);
		...
		Mqtt3MessageHandler petDeviceMessageHandler(Mqtt3AsyncClient mqtt3AsyncClient) {
			var messageHandler = new Mqtt3MessageHandler(mqtt3AsyncClient);
			messageHandler.setMqttConnectView(Mqtt3ConnectView.DEFAULT);
